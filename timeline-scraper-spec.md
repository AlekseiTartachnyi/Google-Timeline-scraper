...
- `flatten` command turns the 7-day JSON into the mileage sheet:
  `date, from_address, departure_time, to_address, arrival_time, miles, route_mi_with_tolls,
  route_mi_no_tolls, mode`. Nine columns, one row per trip, a blank line between days — narrower than the lossless schema in §7 on
...
5. **Route distance cross-check (done, post-M4 — the numbers, not the review).** Timeline reports the length of the
   ...
   **Answered.** The provider is the Google Routes API (`computeRoutes`): the sheet is read
   ...
   asked for. `flatten --routes` fills two columns per trip — `route_mi_with_tolls` from the
   default route and `route_mi_no_tolls` from the same request with
